...
func DealSql(instanceKey string) {
	sqlstring := strings.Split(instanceKey, ":")
	fromWhere := sqlstring[0]
	sql := sqlstring[1]
	if fromWhere == "source" {
		sourceCrcResult := QueryChecksum(Source_db, sql)
		RecordCrcResult(Crc_db, sourceCrcResult, "source")
	} else if fromWhere == "target" {
		targetCrcResult := QueryChecksum(Target_db, sql)
