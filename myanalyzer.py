...
def connect(conf='~/.my.cnf', section='DEFAULT', host_ip=""):
    """
...
def get_bigtransactions(cursor):
    sql = "select trx_id,trx_state,trx_started,trx_requested_lock_id," \
          ...
          "and a.trx_started < date_sub(now(), interval {%d} minute)".format(BIG_TRANSACTION_TIME)
    cursor.execute(sql)
    rows = cursor.fetchall()
...
def block_thread_info(cursor,id):
    check_block_sql = "SELECT r.trx_mysql_thread_id                     AS waiting_thread," \
                      ...
                      "where  r.trx_mysql_thread_id={:d}".format(int(id))
...
def output_sql_table_format(rows):
    data = []
    for data in rows:
        data.append((data["ID"],data["USER"],data["HOST"],data["DB"],data["COMMAND"],data["TIME"],data["STATE"],data["INFO"],
...
def  analyse_processlist(db,outfile,time=10):
    ...
    for row in results:
        ...
        if prefix.startswith("update") or prefix.startswith("delete") or prefix.startswith("insert") and row["STATE"] in ["update","updating","deleting from main table","deleting from reference tables","executing","updating main table","updating reference tables"]:
            dml_long_query.append(row)
        elif prefix.startswith("select") and row["STATE"] in ["Sending data","Copying to tmp table","Copying to tmp table on disk","Creating sort index","executing","Sorting for group","Sorting for order","Sorting result",]:
            select_long_query.append(row)
        elif (prefix.startswith("alter") or prefix.startswith("drop") or prefix.startswith("truncate")) and row["STATE"] in ["altering table","copy to tmp table","Creating index","committing alter table to storage engine","discard_or_import_tablespace","preparing for alter table"]:
    ...
    for row in results:
        ...
        if row["STATE"] == "Waiting for global read lock":
            msg.fail("FLUSH TABLES WITH READ LOCK is waiting for a global read lock or the global read_only system variable is being set.")
        ...
        if row["STATE"] == "Waiting for table flush":
            msg.fail("id :{}  {}is blocked by lock table or long queries.waiting {} seconds ".format(row["ID"],row["INFO"],row["TIME"]))
            #show lock table thread and long query info
            show_long_query(select_long_query, dml_long_query, ddl_long_query)
            show_big_transaction(bigtrx)
        if row["STATE"] == "Waiting for table metadata lock":
            msg.fail("id :{} {} is waiting for table metadata lock. waiting {} seconds".format(row["ID"], row["INFO"], row["TIME"]))
        ...
        if row["STATE"] == "updating":
            ...
            if len(get_block_info) >0:
                ...
                for blockinfo in get_block_info:
                    data.append((blockinfo["waiting_thread"],blockinfo["waiting_query"],blockinfo["waiting_rows_modified"],
                ...
                header = ("waiting_thread", "waiting_query", "waiting_rows_modified","waiting_age", "waiting_wait_secs", "waiting_user",
        ...
        if row["STATE"] == "Opening tables":
            ...
            if open_table_count > 10:
                msg.fail("increase table_open_cache or decrease tables in sql or decrease sql parrallel execution or disk performance is not good")
...
def main():
    ...
    try:
        ...
        if opts.host:
            ...
            host = ""
        con = connect(opts.config, opts.section,host)
    except Exception, e:
        print("errors:%s" % e)
